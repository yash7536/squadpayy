...
describe("splitByItem", () => {
  ...
  test("a genuinely empty assignment ([]) contributes nothing — unlike a missing key", () => {
    // Explicitly-empty is a real UI state ("not assigned yet"), distinct
    // from no assignment info at all — it must NOT fall back to everyone,
    // or "Split by item" would silently match "Split equally" by default.
    const result = splitByItem(items, people, { Drinks: [] });
    const drinksOwed = 400; // Drinks' full price, unclaimed by anyone
    assert.equal(sumAmounts(result), 2400 - drinksOwed);
    // Dinner and Service Charge still have no assignment key at all, so
    // they still fall back to everyone as before.
    for (const { amount } of result) {
      assert.ok(amount > 0);
    }
  });

  test("every item genuinely unassigned sums to zero, not the full total", () => {
    const result = splitByItem(items, people, {
      Dinner: [],
      Drinks: [],
      "Service Charge": [],
    });
    assert.equal(sumAmounts(result), 0);
    assert.deepEqual(
  ...
  test("assigning items one at a time recalculates incrementally toward the full total", () => {
    const halfway = splitByItem(items, people, { Dinner: ["You"], Drinks: [], "Service Charge": [] });
    assert.equal(sumAmounts(halfway), 1800); // only Dinner assigned so far
    const fullyAssigned = splitByItem(items, people, {
      Dinner: ["You"],
      Drinks: ["Rahul"],
      "Service Charge": ["Ananya"],
    });
    assert.equal(sumAmounts(fullyAssigned), 2400); // now everything's claimed
  });
...
describe("splitBillWithTaxAndTip", () => {
  ...
  test("an unassigned item leaves the total short by its price, tax/tip still exact", () => {
    // Drinks genuinely unassigned ([]) — its Rs.400 isn't anyone's yet.
    const result = splitBillWithTaxAndTip({
      items,
      tax: 120,
      tip: 300,
    ...
    // Short by exactly Drinks' price; tax/tip (120 + 300) still fully
    // allocated on top of whatever subtotal IS currently assigned.
    assert.equal(sumAmounts(result), 2400 - 400 + 120 + 300);
  });
