...
// for — populated by Review Bill and read by Split Bill/Review & Send.
// Starts empty: a brand-new bill (whether scanned or entered manually)
// starts with nothing until Review Bill actually populates it — either
// automatically from a real AI scan, or by the sender adding items
// themselves for "Enter manually" (see app/review-bill/page.js). It used
// to default to a fixed mock example ("Dinner", "Drinks", ...) even for a
// fresh manual entry, which meant "start empty" never actually happened —
// that's the bug this fixes.
// itemAssignments maps an item's name to the participants currently
// assigned to it for "Split by item" (e.g. { Pizza: ["You", "Rahul"] }) —
// see app/review-send/ReviewSendClient.js and lib/splitBill.js's
// splitByItem for how a missing/empty entry is treated. Persisted here
// (not local component state) so it survives navigating away from Review
// & Send and back — e.g. to Sign in and back, which used to silently
// reset every item back to unassigned since it only ever lived in a
// useState tied to that one component's mount.
const DEFAULT_DRAFT = {
  splitMethod: "equal",
...
export function useBillDraft() {
  ...
  // Called once by Review Bill when a real AI scan comes back, to record
  // the confirmed items/tax/tip/total in one shot — see that page for why
  // every amount here is a whole rupee (Math.round), matching the
  // whole-rupee assumption baked into lib/splitBill.js's remainder-rotation
  // math.
  ...
  // Used by "Enter manually" on Review Bill, where the sender builds the
  // item list themselves one add/edit/remove at a time rather than
  // confirming an already-extracted receipt in one shot. Recomputes
  // billTotal from the items every time — manual entry has no separate
  // tax/tip concept (billTax/billTip are left as whatever they already
  // are, i.e. 0 unless a scan set them), so the total is just the items'
  // sum, same as it's always been for a manually-built bill.
  function setBillItems(items) {
    const total = items.reduce((sum, item) => sum + item.amount, 0);
    writeDraft({ ...readDraft(), billItems: items, billTotal: total });
  }

  // Same updater-function convention as setContacts, so ReviewSendClient's
  // existing toggleAssignment logic (a functional setState update) needs
  // no change beyond calling this instead of a local setter.
  function setItemAssignments(update) {
    const current = readDraft();
    const nextAssignments =
      typeof update === "function"
        ? update(current.itemAssignments)
        : update;
    writeDraft({ ...current, itemAssignments: nextAssignments });
  }
